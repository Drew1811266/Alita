...
def test_planner_chain_preserves_tool_catalog_dag_nodes() -> None:
    message = UserMessage(
        task_id="task-tool-catalog-dag-chain",
        content=(
            "Use the document read write tool and echo values tool to read "
            "then echo the extracted text."
        ),
    )
    request = _request_for(
        message,
    ...
    metadata = result.graph_payload["metadata"]["toolCatalogPlanner"]
    assert metadata["toolIds"] == [
        "internal:document.read_write",
        "internal:test.echo_values",
    ]
    assert metadata["operations"] == ["read", "echo_values"]
