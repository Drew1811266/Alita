...
def build_research_graph(
    ...
    nodes = [
        ...
        _node(
            node_id="research-source-reading",
            node_type="fixed_tool",
            display_name="Source reading",
            status="waiting",
            input_ports=[_port("source-set-input", "Accepted and rejected sources", "json")],
            output_ports=[_port("source-content-output", "Read source content", "json")],
            dependencies=["research-source-review"],
            summary="Fetches accepted sources so synthesis is based on page content, not only search snippets.",
            position={"x": 90, "y": 820},
            tool_ref="web.fetch.sources",
            estimate=_estimate(6000, "low", "medium", "required"),
            resource_usage={"cpu": "low", "memory": "medium", "network": "required"},
        ),
        _node(
            ...
            status="waiting",
            input_ports=[_port("source-content-input", "Read source content", "json")],
            output_ports=[_port("report-output", "Research report", "text")],
            dependencies=["research-source-reading"],
            summary="Synthesizes reviewed and read sources into the confirmed report section order.",
            position={"x": 430, "y": 820},
            model_ref="research-report-synthesizer",
            estimate=_estimate(2500, "medium", "medium", "none"),
        ),
        _node(
            node_id="research-report-quality-check",
            node_type="model",
            display_name="Report quality check",
            status="waiting",
            input_ports=[_port("report-input", "Research report", "text")],
            output_ports=[_port("quality-output", "Report quality diagnostics", "json")],
            dependencies=["research-report-synthesis"],
            summary="Checks the synthesized report for source coverage and obvious empty-output failures.",
            position={"x": 260, "y": 990},
            model_ref="research-report-verifier",
            estimate=_estimate(900, "low", "low", "none"),
        ),
        _node(
            ...
            dependencies=["research-report-quality-check"],
            summary="Publishes the research report as Markdown.",
            position={"x": 260, "y": 1150},
