...
当前版本仍处于早期开发阶段，但已经形成了一个可运行的最小闭环：用户可以创建或打开 `.alita` 工程，在聊天区向 Agent 提需求，添加本地文档附件，让系统生成并运行文档处理节点流程；也可以在首选项里管理本地模型，把 Agent 模型和语音转文字模型统一配置到模型库中。

## 当前定位

Alita 的产品方向是“本地 AI 生产力工作台”：

- **本地优先**：Agent LLM、语音转文字模型和文档处理工具优先在用户电脑本地运行，减少对外部云服务的依赖。
- **工程化协作**：聊天记录、附件引用、节点流程、运行历史和产物会保存到 `.alita` 工程文件中。
- **节点化任务执行**：Agent 不只是回复文本，还可以把文档处理任务拆成可观察、可运行、可重试的节点流程。
- **可扩展模型库**：软件首选项里维护统一模型库，目前包含 Agent GGUF 模型和语音转文字模型，后续可以继续扩展到其他本地模型模块。
...
- 普通聊天请求会交给本地 Agent 模型生成回复。
- 文档处理类请求会生成节点流程。
- 当用户提到需要处理文档但没有提供附件时，系统会要求补充文件。

当前 Agent 模型通过本地 `llama.cpp` runtime 调用，模型文件使用 GGUF 格式。
...
首选项中已经有统一的模型库视图，用来管理软件需要调用的本地模型。当前支持两类模型：
...
Agent LLM 当前通过 `llama.cpp` 的 `llama-server.exe` 运行。开发脚本会在配置了 Agent 模型后自动启动本地模型服务，默认地址是：
