...
运行历史记录每次 run 的状态、节点 run record、artifact refs 和 runtime notice。运行时还会写入 checkpoint、runtime span、恢复审计记录，并在前端 controller 层保留 checkpoint、authority decision 和 recovery action 观测状态。
