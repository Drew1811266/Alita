...
当前仓库版本为 `0.31.0`。这个阶段的重点是：桌面工程闭环已经成型，并完成 Agent Runtime A-L 阶段优化以及 closed-loop runtime 加固：Agent 具备运行状态、统一工具网关、结构化路由、规划链、执行图、显式权限授权、manifest 驱动工具入口、受控 ReAct、运行 checkpoint、低风险恢复继续、证据驱动研究、评估基线、项目记忆上下文和前端 runtime 观测状态。项目仍处于开发期，不是稳定发行版，但已经不再只是 UI 原型。
...
- 可以为节点运行写入 checkpoint，并在低风险失败补丁建议下进行一次受控自动继续。
...
运行历史记录每次 run 的状态、节点 run record、artifact refs 和 runtime notice。运行时还会写入 checkpoint、恢复审计记录，并在前端 controller 层保留 checkpoint、authority decision 和 recovery action 观测状态。
...
- run checkpoint 和低风险自动继续已经存在；从 checkpoint 恢复、rollback、后台多 run 队列和 memory 管理 UI 仍属于后续增强项。
